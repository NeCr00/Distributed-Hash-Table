...
class node:
    ...
    def __init__(self, ip, port):
        ...
        self.thread = threading.Thread(target=self.run, args=(self.q,))  # creates a thread to run the node
        self.thread.start()  # starts the thread

    def run(self, queue):  # run () is the function that proccess is executing when you run the thread.start()
    ...
    def find_successor(self, id):
        ...
        if(id > self.id & id <= self.successor["id"]):
    ...
    def stabilize (self):
        ...
        if(predecessor_id> self.id & predecessor_id<self.successor["id"]):
            self.successor = {"id":response["id"],"ip":response["id"],"port":response["port"]}
        
        data = {"command":"notify","id":self.id,"ip":self.ip,"port":self.port}
        self.sendData(self.successor["ip"],self.successor["port"],data)

    def notify(self,node):
        if(self.predecessor==None | (node["id"]>self.predecessor["id"] * node["id"]<self.id)):
